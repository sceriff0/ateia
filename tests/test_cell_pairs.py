...
def test_multipolygon_contributes_one_ring_per_part():
    ps = cp.from_feature_collection(_fc(_multipolygon(_square(0, 0, 4), _square(20, 0, 4))))
...
def test_median_equivalent_radius_matches_a_disc():
    r = 7.0
    assert cp.median_equivalent_radius(np.array([math.pi * r * r] * 5)) == pytest.approx(r)
...
def test_summarize_stage_adds_micron_stats_only_with_a_pixel_size():
    iou = np.array([1.0])
    rec_px = cp.summarize_stage(iou, np.array([True]), np.array([2.0]),
                                np.array([1.0]), np.array([1.0]))
    assert "displacement_um_p50" not in rec_px
    rec_um = cp.summarize_stage(iou, np.array([True]), np.array([2.0]),
                                np.array([1.0]), np.array([1.0]), pixel_size_um=0.5)
...
def test_summarize_stage_dice_matched_matches_the_closed_form():
    ...
    rec = cp.summarize_stage(np.array([1.0, 1.0]), np.array([True, True]), np.array([0.0, 0.0]),
    ...
    rec2 = cp.summarize_stage(np.array([1.0 / 3.0]), np.array([True]), np.array([0.0]),
...
def test_summarize_stage_excludes_unscored_pairs_but_counts_them():
    rec = cp.summarize_stage(np.array([1.0, np.nan]), np.array([True, False]),
                             np.array([0.0, 5.0]), np.array([1.0, 1.0]), np.array([1.0, 1.0]))
...
def test_summarize_stage_of_no_pairs_is_degenerate_not_a_crash():
    rec = cp.summarize_stage(np.empty(0), np.empty(0, dtype=bool), np.empty(0),
...
def test_a_fixed_pairing_tracks_the_same_cells_through_successive_warps():
    ...
    mov_native = cp.from_feature_collection(_fc(*[_square(i * 40 + 7, 0, 10) for i in range(6)]))
