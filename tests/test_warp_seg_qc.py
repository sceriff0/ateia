...
from utils.valis_stage_warp import STAGE_MICRO, STAGE_NATIVE, STAGE_NON_RIGID, STAGE_RIGID
...
def test_run_fixes_the_pairing_at_the_anchor_not_per_stage(tmp_path):
    ...
    warp = _shift_warp({STAGE_NATIVE: 0.0, STAGE_RIGID: -96.0,
                        STAGE_NON_RIGID: -99.0, STAGE_MICRO: -100.0})

    out = wsq.run(ref, mov, warp,
                  [STAGE_NATIVE, STAGE_RIGID, STAGE_NON_RIGID, STAGE_MICRO],
                  ref_slide=REF, moving_slide=MOV)
...
def test_run_pairs_at_the_anchor_even_when_native_is_unmatchable(tmp_path):
    ...
    out = wsq.run(ref, mov, warp, [STAGE_NATIVE, STAGE_RIGID, STAGE_MICRO],
                  ref_slide=REF, moving_slide=MOV)
...
def test_match_radius_is_derived_from_the_cells_themselves(tmp_path):
    ...
    out = wsq.run(ref, mov, warp, [STAGE_RIGID, STAGE_MICRO], ref_slide=REF, moving_slide=MOV)
    # A 10x10 square has equivalent radius sqrt(100/pi) ~= 5.64; factor 1.0 -> that radius.
    assert out["matching"]["median_cell_radius_px"] == pytest.approx(math.sqrt(100 / math.pi))
    assert out["matching"]["radius_px"] == pytest.approx(out["matching"]["median_cell_radius_px"])
...
def test_successive_stages_improve_iou_and_shrink_displacement(tmp_path):
    ...
    assert s[STAGE_RIGID]["iou_mean"] < s[STAGE_NON_RIGID]["iou_mean"] < s[STAGE_MICRO]["iou_mean"]
...
def test_deltas_are_measured_against_the_anchor_and_carry_the_sign(tmp_path):
    ...
    assert STAGE_RIGID not in out["delta_vs_anchor"]          # the anchor has no delta to itself
    ...
    assert d[STAGE_NATIVE]["displacement_px_p50"] == pytest.approx(+96.0)  # native is worse
...
def test_a_stage_that_makes_things_worse_shows_a_positive_displacement_delta(tmp_path):
    ...
    ref = _write(tmp_path, "ref.geojson", _grid_fc())
    mov = _write(tmp_path, "mov.geojson", _grid_fc(dx=100.0))
    warp = _shift_warp({STAGE_RIGID: -100.0, STAGE_NON_RIGID: -100.0, STAGE_MICRO: -97.0})

    out = wsq.run(ref, mov, warp, [STAGE_RIGID, STAGE_NON_RIGID, STAGE_MICRO],
                  ref_slide=REF, moving_slide=MOV, supersample=4)

    assert out["delta_vs_anchor"][STAGE_NON_RIGID]["displacement_px_p50"] == pytest.approx(0.0)
    assert out["delta_vs_anchor"][STAGE_MICRO]["displacement_px_p50"] == pytest.approx(+3.0)
...
def test_micron_stats_appear_only_with_a_pixel_size(tmp_path):
    ...
    um = wsq.run(ref, mov, warp, [STAGE_RIGID], ref_slide=REF, moving_slide=MOV,
                 pixel_size_um=0.325)
    assert um["stages"][STAGE_RIGID]["displacement_um_p50"] == pytest.approx(2.0 * 0.325)
...
def test_run_rejects_a_stage_plan_without_the_anchor(tmp_path):
    ...
    with pytest.raises(ValueError, match="anchor stage"):
        wsq.run(ref, mov, _shift_warp({STAGE_NATIVE: 0.0}), [STAGE_NATIVE],
                ref_slide=REF, moving_slide=MOV)
...
def test_write_report_emits_the_documented_record(tmp_path):
    ...
    rec = wsq.write_report(None, REF, MOV, ref, mov, str(out_f), patient_id="p1",
                           warp=warp, stages=[STAGE_NATIVE, STAGE_RIGID, STAGE_MICRO],
                           separable=False, note="test note")
...
def test_write_report_defaults_display_names_to_the_slide_names(tmp_path):
    ref = _write(tmp_path, "ref.geojson", _grid_fc())
    mov = _write(tmp_path, "mov.geojson", _grid_fc(dx=1.0))
    rec = wsq.write_report(None, REF, MOV, ref, mov, str(tmp_path / "o.json"),
                           warp=_shift_warp({STAGE_RIGID: 0.0}), stages=[STAGE_RIGID])
    assert rec["moving"] == MOV and rec["reference"] == REF
...
def test_each_planned_stage_is_warped_for_both_slides_and_no_others(tmp_path):
    ...
    wsq.run(ref, mov, warp, [STAGE_NATIVE, STAGE_RIGID, STAGE_MICRO],
            ref_slide=REF, moving_slide=MOV)
...
def test_parse_args_defaults_match_the_documented_behaviour():
    a = wsq.parse_args(["--pickle", "p.pickle", "--ref-slide", "r", "--moving-slide", "m",
