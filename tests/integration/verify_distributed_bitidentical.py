...
def main():
    ...
    n = manifest["n_tiles"]
    print(f"[verify] grid n_tiles={n} ({manifest['n_rows']}x{manifest['n_cols']})", flush=True)
    for i in range(n):
    ...
    mask = pyvips.Image.new_from_file(os.path.join(ti, "mask.v")) if manifest["has_mask"] else None
    reg = NonRigidTileRegistrar(tile_wh=TILE_WH, tile_buffer=TILE_BUFFER)
    _, _, in_proc_bk = reg.register(moving, fixed, mask=mask, non_rigid_registrar_cls=OpticalFlowWarper,
    ...
    moving_bk = in_proc_bk if isinstance(in_proc_bk, pyvips.Image) else \
        warp_tools.numpy2vips(np.dstack([np.asarray(in_proc_bk[0]), np.asarray(in_proc_bk[1])])).cast("float")
    incoming = pyvips.Image.black(moving_bk.width, moving_bk.height, bands=2).cast("float")
    self_bk = reg_finalize.compose(moving_bk, incoming, reg_mask, from_rigid_reg=ws.get("from_rigid_reg", False))
    ...
    slide_bk = pyvips.Image.black(os_rc[1], os_rc[0], bands=2).cast("float").insert(
        reg_finalize._to_vips_field(self_bk), pb[0], pb[1])
    base_warped = slide_tools.warp_slide(
        ...
        M=np.asarray(ws["M"]), dxdy=slide_bk, level=0, series=ws.get("series"),
    ...
    if dist_px.shape != base_px.shape:
        print(f"[verify] SHAPE MISMATCH dist={dist_px.shape} base={base_px.shape}", flush=True)
        return 1
    equal = np.array_equal(dist_px, base_px)
    d = float(np.max(np.abs(dist_px - base_px)))
    print("\n" + "=" * 72)
    print(f"distributed vs in-process tiler (same 2-D inputs): equal={equal} max|Δ|={d}")
