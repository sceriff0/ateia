...
def test_can_read_accepts_pyramidal_and_reads_full_res():
    ...
    with tifffile.TiffWriter(p, bigtiff=True, ome=True) as tif:
        ...
        tif.write(
            ...
            metadata={
                "axes": "CYX",
                "Channel": {"Name": ["DAPI", "SMA", "PANCK", "CD3"]},
            },
            **options,
        )
    ...
    with tifffile.TiffFile(p) as tf:
        assert tf.pages[0].subifds, (
            "fixture did not actually produce a pyramidal page 0"
        )
...
def test_valis_subifd_pyramid_reads_full_res_all_channels():
    ...
    with tifffile.TiffFile(path) as tf:
        assert tf.pages[0].subifds, (
            "fixture is not a sub-IFD pyramid; test would be vacuous"
        )
