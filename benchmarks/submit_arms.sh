...
EXTRA_ARGS=()
# CSE is turned on by a PROFILE, never by `--skip_seg_quality_eval false`.
# Nextflow 26 delivers every --param as a String, so that flag would set the param
# to the string "false" -- which is truthy, and would disable the scorer while
# reading like it enables it. tests/test_no_cli_boolean_params_in_docs.py guards it.
