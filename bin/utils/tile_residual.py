...
tile's local Target Registration Error (the magnitude of that residual).
...
def residual_displacement(ref_tile, mov_tile, upsample=10, whiten_sigma=3.0):
    ...
    Returns ``(dx, dy, tre)``: ``(dx, dy)`` is the control-point displacement (add it to a moving
    coordinate to reach the reference), and ``tre = hypot(dx, dy)`` is the tile's local rigid-stage
    TRE — the pre-refinement misalignment magnitude used to gate whether the tile needs refining.
