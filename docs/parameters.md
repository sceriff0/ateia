...
| `reg_tiled_frontend` | `orb` | COARSE global-alignment front-end: `orb` (default, today's behaviour) \| `sift` \| `fourier_mellin` \| `disk_lightglue`. Not tier-owned. `disk_lightglue` needs torch+kornia. They ship in the `tiled` container (`containers/tiled/Dockerfile`), together with both pretrained checkpoints baked in under `TORCH_HOME=/opt/torch`, so it needs no extra profile, no second image and no download at first use. Outside a container, install `torch==2.3.1` (CPU wheel) and `kornia==0.7.3` or selecting it fails fast with a clear `RuntimeError`. It is by far the most memory-hungry front-end — DISK is a U-Net, so peak memory scales with thumbnail AREA rather than keypoint count: 3.03 GB at 512 px and 8.78 GB at 1024 px on the pinned stack, ~20x ORB at equal size. `orb` remains the default. |
