...
Generated after the `preprocessing` step. Use as `--input` when running `--start registration`.
...
Generated after the `registration` step. Use as `--input` when running `--start postprocessing`.
