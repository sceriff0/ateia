...
| `channel_stem` | `quantify_markers.nf`, from the split channel TIFF's basename (the SANITISED form, e.g. `HLA_DR` for a declared `HLA.DR`) | The filename-safe stem for the marker this task is quantifying. Read only by `QUANTIFY`'s `publishDir`, to keep the published `<patient>_<stem>_quant.csv` filenames byte-identical to what the pre-batching per-marker fan-out produced. Deliberately distinct from `channel_name`, which carries the DECLARED panel name and fills the measurement key — see `lib/ChannelName.groovy`. |
