...
| `segeval` | `bolt3x/attend_image_analysis:segeval` | `SEG_QUALITY_EVAL`, `MERGE_SEG_EVAL` (opt-in, `--skip_seg_quality_eval false`) | `python:3.11-slim` + numpy/scipy/pandas/scikit-image/scikit-learn/aicsimageio/tifffile/xmltodict (vendored CSE 1.5.19 subset under `bin/utils/cse/`) |
...
`REGISTER` uses the upstream `cdgatenbee/valis-wsi:1.0.0`.
